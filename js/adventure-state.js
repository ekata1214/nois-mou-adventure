...
export const CHESTS=[{id:'wind',name:'風の眼球',x:-5,z:-18},{id:'pond',name:'水辺の眼球',x:-29,z:-48},{id:'shrine',name:'祠の眼球',x:-65,z:-68}];
...
export function nextAdventureGoal(a){
  if(!a.chests.includes('wind'))return {id:'wind',text:'近くの眼球に触れてみよう'};
  if(a.runes.length<3)return {id:RUNES.find(r=>!a.runes.includes(r.id)).id,text:`丘の祠で、3つの灯りをともそう（${a.runes.length}/3）`};
  if(!a.chests.includes('shrine'))return {id:'shrine',text:'灯りがそろった。祠の眼球を開こう'};
