...
export function buildFieldSky(scene){
  ...
  for(let y=0;y<height;y++)for(let x=0;x<width;x++){
    ...
    const base=[145-53*h,143-54*h,153-52*h],shade=160+n*28;
  ...
  const moon=new THREE.Mesh(new THREE.SphereGeometry(13,32,20),new THREE.MeshBasicMaterial({color:0x8b8993,fog:false,toneMapped:false}));moon.position.set(-115,110,-165);scene.add(moon);
  const orbit=new THREE.Mesh(new THREE.TorusGeometry(20,.35,8,80),new THREE.MeshBasicMaterial({color:0xaaa1a3,fog:false}));orbit.position.copy(moon.position);orbit.rotation.set(.7,.3,.6);scene.add(orbit);
