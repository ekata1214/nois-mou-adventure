...
import {createSootShard} from './uncanny-props.js?v=20260909eye';
...
export function createShellLifeView(player,onCollect,npcs=[]){
 ...
 const spots=[[-1.8,.7],[-.5,1.4],[1.2,.8],[2.2,-.1],[.1,-.2],[-1.3,-.4]].map(([x,z],i)=>{const m=createSootShard(i);m.scale.setScalar(.55);m.position.set(x,.3,z);m.traverse(part=>part.userData.spot=i);scene.add(m);return m;});
 ...
 touch(event,canvas){const r=canvas.getBoundingClientRect(),ray=new THREE.Raycaster();ray.setFromCamera(new THREE.Vector2((event.clientX-r.left)/r.width*2-1,-(event.clientY-r.top)/r.height*2+1),camera);const hit=ray.intersectObjects(spots.filter(m=>m.visible),true)[0];if(hit)this.collect(hit.object.userData.spot);},
 update(dt,s,paused,friends={},lit=false){
  guests.forEach(g=>g.sprite.visible=friends[g.id]==='home');lamp.intensity=lit?12:0;
  spots.forEach((m,i)=>{m.visible=s.spots[i]<=Date.now();m.userData.animate(time);m.position.y=.3+Math.sin(time*2+i)*.055;});
