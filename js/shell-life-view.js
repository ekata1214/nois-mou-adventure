...
export function createShellLifeView(player,onCollect,npcs=[]){
 const scene=new THREE.Scene();scene.background=new THREE.Color('#100d12');scene.fog=new THREE.FogExp2('#100d12',.025);
 const camera=new THREE.PerspectiveCamera(43,1,.1,60);camera.position.set(7,7,10);camera.lookAt(0,.65,0);
 scene.add(new THREE.HemisphereLight(0xe5c8b9,0x281722,2));const light=new THREE.PointLight(0xff665c,35,16);light.position.set(-2,4,1);scene.add(light);
 ...
 box(0,-.18,0,7,.36,6,0x272329);box(0,1.5,-3,7,3,.18,0x282027);box(-3.5,1.1,0,.18,2.2,6,0x39242c);
 for(let i=-3;i<=3;i++)box(i,-.005,0,.012,.012,6,0x695254);
 const furnitureStart=scene.children.length;
 box(-2,.22,-1.8,1.5,.44,1.8,0x561c29);box(-2,.5,-2.3,1.3,.18,.6,0xa98b7c);
 box(1.9,.55,-2,1.8,.12,1,0x6e5448);for(const x of [1.15,2.65])box(x,.25,-2,.08,.5,.8,0x3c3034);
 const furniture=scene.children.slice(furnitureStart);
 new GLTFLoader().load('assets/room/shell-lite.glb',gltf=>{furniture.forEach(m=>scene.remove(m));scene.add(gltf.scene);},undefined,()=>{});
 const window=box(-.1,1.9,-2.88,1.4,1.6,.03,0x120e17);window.material.emissive.set(0x801622);window.material.emissiveIntensity=.6;
 const ring=new THREE.Mesh(new THREE.TorusGeometry(.48,.04,8,40),new THREE.MeshBasicMaterial({color:0xe50914}));ring.position.set(-.1,1.95,-2.83);scene.add(ring);
 ...
 const shelf=[];for(let i=0;i<12;i++){
  const vessel=new THREE.Mesh(new THREE.IcosahedronGeometry(.14,1),mat.clone());vessel.position.set(-2.9,.2,.1+(i%6)*.38);scene.add(vessel);
  const paper=new THREE.Mesh(new THREE.PlaneGeometry(.62,.42),new THREE.MeshBasicMaterial({color:0xffffff}));paper.position.set(-3.39,.85+Math.floor(i/4)*.55,-.65+(i%4)*.72);paper.rotation.y=Math.PI/2;scene.add(paper);shelf.push({vessel,paper,text:null});
 }
 const guests=npcs.map((n,i)=>{const sprite=n.obj.clone();sprite.position.set(-2.9+i*1.2,.9,-2.5);sprite.scale.set(1,1,1);scene.add(sprite);return {sprite,id:n.id};});
 ...
 update(dt,s,paused,friends={},lit=false){
  ...
  if(paused)return false;time+=dt;
  ...
  return moving;
 },resize(w,h){camera.aspect=w/h;camera.position.set(w<h?9:7,w<h?10:7,w<h?15:10);camera.lookAt(0,w<h?-2:.65,0);camera.updateProjectionMatrix();}};
}
